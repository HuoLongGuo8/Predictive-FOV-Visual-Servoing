    ...
    , session_update_(nullptr)
{
    ...
    auto ensure_onnx_suffix = [](const std::string& path) {
    ...
    std::string init_path = ensure_onnx_suffix(model_init_str);
    std::string update_path = ensure_onnx_suffix(model_update_str);
